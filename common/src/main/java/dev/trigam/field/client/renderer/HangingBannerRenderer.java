...
public class HangingBannerRenderer implements BlockEntityRenderer<HangingBannerBlockEntity> {
    public static final Material SPRITE = new Material(
            Sheets.BANNER_SHEET,
            ResourceLocation.fromNamespaceAndPath(Constants.MOD_ID, "entity/banner/base/hanging_banner_base")
    );
    ...
    public HangingBannerRenderer(BlockEntityRendererProvider.Context context) {
        this.bannerModel = new HangingBannerModel(context.bakeLayer(EntityModelLayerInit.HANGING_BANNER));
        this.flagModel = new BannerFlagModel(context.bakeLayer(EntityModelLayerInit.BANNER_FLAG));
    }

    @Override
    public void render(
            @NotNull HangingBannerBlockEntity banner, float tickDelta, @NotNull PoseStack matrices,
    ...
    public void renderFrame(
            HangingBannerBlockEntity banner, PoseStack matrices,
            MultiBufferSource vertexProvider, int light, int overlay
    ) {
        matrices.pushPose();

        matrices.translate(0.5F, 0.0F, 0.5F);
        float rotation = -RotationSegment.convertToDegrees(banner.getBlockState().getValue(BannerBlock.ROTATION));
